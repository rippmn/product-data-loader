...
import com.rippmn.product.loader.search.DocumentSearchService;
...
public class FileLoader {
	...
	public String indexFile(String bucket, String loadFileName) throws JsonParseException, IOException {

		GcsFilename fileName = new GcsFilename(bucket, loadFileName);
		GcsInputChannel readChannel = gcsService.openPrefetchingReadChannel(fileName, 0, BUFFER_SIZE);
		InputStream in = Channels.newInputStream(readChannel);
		...
		while (!parser.isClosed()) {
			...
			if (JsonToken.START_ARRAY.equals(token)) {
				...
				if (products % 10 == 0) {
					...
					documentSearchService.indexDocs(prodSaveList);
